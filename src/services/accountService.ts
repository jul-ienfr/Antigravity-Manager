...
export async function listOAuthClients(): Promise<OAuthClientInfo[]> {
    const res = await invoke<OAuthClientInfo[] | { clients: OAuthClientInfo[] }>('list_oauth_clients');
    if (res && !Array.isArray(res) && Array.isArray((res as any).clients)) return (res as any).clients;
    return (res as OAuthClientInfo[]) || [];
}
